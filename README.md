...
- **メディアビューアー** — プレビュー内の画像・Mermaid SVG をクリックするとライトボックス表示。ホイールでズーム（0.1〜10倍）、ドラッグでパン、ダブルクリックでリセット、Escape・背景クリック・✕ボタンで閉じる
- **スクロール同期の改善** — Split モードのスクロール同期をライン番号マッピング方式に改善し、精度を向上

#### Fixed

- メディアビューアーを閉じた際にプレビューのスクロール位置がリセットされる問題を修正（`components` オブジェクトの `useMemo` メモ化により解消）
...
| メディアビューアー | 画像・Mermaid SVG をクリックしてライトボックス表示。ズーム・パン・リセット対応 |
