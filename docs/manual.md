...
| ショートカット | クリップボードの内容 | 動作 |
| --- | --- | --- |
| `Ctrl+V` | テキスト | テキストをそのまま貼り付け |
| `Ctrl+Shift+V` | タブ区切りテキスト（Excel セル） | Markdown テーブルとして挿入 |
| `Ctrl+Alt+V` | 画像 | 画像ファイルとして保存してリンク挿入 |
| `Ctrl+Alt+V` | テキストのみ | 何もしない |

> **注意:** Excel からセル範囲をコピーしたとき、`Ctrl+V` はテキスト貼り付け、`Ctrl+Alt+V` は画像貼り付けとして動作します。
...
- **GFM（GitHub Flavored Markdown）**: テーブル、チェックボックス（プレビュー側でクリックして切替可能）、打ち消し線など
...
| `Ctrl+V` | テキスト貼り付け（通常の貼り付け） |
| `Ctrl+Shift+V` | タブ区切りテキスト（Excel コピー）を Markdown テーブルとして貼り付け |
| `Ctrl+Alt+V` | クリップボード内の画像をファイルとして保存してリンク挿入 |
| `Ctrl+;` | 現在の日付を挿入（例: `2026/3/29`） |
| `Ctrl+:` | 現在の時刻を挿入（例: `14:05`） |
| `(` | `()` を挿入してカーソルを内側に移動 |
| `[` | `[]` を挿入してカーソルを内側に移動 |
| `` ` `` | ` `` ` ` を挿入してカーソルを内側に移動 |
...
プレビュー（スプリットプレビュー含む）でチェックボックスをクリックすると、エディタ側の Markdown テキストが即座に切り替わります。

```markdown
- [ ] 未完了のタスク   ← クリックすると下に切り替わる
- [x] 完了済みのタスク ← クリックすると上に切り替わる
```

- エディタを開いていない「プレビューモード」でも操作可能です
- 変更は即時にプレビューへ反映されます
- `Ctrl+Z` で元に戻せます

### TOC（目次）とコードブロック

TOC サイドバーはコードブロック内の `#` 始まり文字列を見出しとして拾わなくなりました。
