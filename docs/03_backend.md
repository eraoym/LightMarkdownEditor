...
> 最終更新: 2026-04-16（Issue #21: 多重起動防止プラグイン追加）
...
| `tauri-plugin-single-instance` | 多重起動防止（Issue #21） |
...
pub fn run() {
    tauri::Builder::default()
        // single-instance は必ずプラグインリストの先頭に置く（Issue #21）
...
| ファイル | 変更内容 |
| --- | --- |
| `src-tauri/Cargo.toml` | `tauri-plugin-fs`, `tauri-plugin-dialog`, `tauri-plugin-single-instance` を追加 |
| `src-tauri/src/lib.rs` | 3プラグインの `.plugin()` 登録 + `get_startup_args` コマンド + 多重起動防止コールバック（Issue #21） |
