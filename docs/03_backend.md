...
| プラグイン | 用途 |
| --- | --- |
| `tauri-plugin-fs` | ファイルの読み込み・書き込み |
...
| `src-tauri/src/lib.rs` | 2プラグインの `.plugin()` 登録 |
| `src-tauri/capabilities/default.json` | fs / dialog のパーミッション追加 |
| `src/App.tsx` | `openFile` / `saveFile` 呼び出しロジック |
